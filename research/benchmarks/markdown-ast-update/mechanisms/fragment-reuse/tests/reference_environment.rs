//! CORRECTNESS_REGRESSION_FIXTURE — H2 reference-environment closure
...
//!
//! Every fixture asserts `H2 update == H0 clean parse(post)` — the frozen
...
fn h2_full_state(bytes: &[u8]) -> H2State {
    ...
    let mechanism = FragmentReuseMechanism::new();
...
fn fence_closer_removal_refreshes_a_reused_payload() {
    ...
    let (result, counters) = h2_update(pre.as_bytes(), post.as_bytes(), start, start + 3, "");
    assert_eq!(
        result,
        reference(post.as_bytes()),
        "H2 update must equal the H0 clean parse of the post source"
    );
    assert!(
...
fn fence_closer_restoration_refreshes_a_reused_payload() {
    ...
    let (result, _) = h2_update(broken.as_bytes(), restored.as_bytes(), start, start, "```");
