//! Frozen 370-slot CASE-MATRIX-v1 differential for H4 RESTART_CONVERGENCE (stage R5).
//!
//! Instantiates the frozen case matrix (`cases/CASE-MATRIX-v1.md`) and
//! runs every UNIQUE case through restart_convergence against the H0 authoritative
//! parse: `H4 result == H0` on every update, on the FULL_PARSE
...
fn full_parse_state(bytes: &[u8]) -> H4State {
