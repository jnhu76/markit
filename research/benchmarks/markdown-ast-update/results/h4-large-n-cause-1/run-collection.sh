...
echo "H4LARGEN_COLLECTION_START utc=$(date -u +%Y-%m-%dT%H:%M:%SZ)"
...
# ---- Lane A: U_PLAIN (original H4) -------------------------------------
for s in 0 1 2; do
  run "a-u-plain-session-$s" mdbench-h4diag \
      run-plain --session "$s" \
      --out "$RAW/u-plain-session-$s.jsonl" \
      --schedule-out "$RAW/u-plain-session-$s.schedule.jsonl"
done
