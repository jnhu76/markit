...
§6/§7 amended by CORRECTIVE-C, PR #39: the reserved
`PERFORMANCE_MATRIX_FREEZE` gate is retired — controlled synthetic work is
observation-driven explanatory work, not a pre-performance gate).
...
Controlled synthetic work is **observation-driven explanatory work** that
follows real measurements; it is not a phase that must complete before
measurement. The authoritative sequence is:

```text
REAL_WORKLOAD_FREEZE
        ↓
#31 real measurements (SURFACE R)
        ↓
material observation / hypothesis
        ↓
#22 controlled synthetic follow-up (SURFACE S/X)
        ↓
N/B/L/K/F causal explanation where needed
```

When such follow-up happens, the mapping step takes frozen real transitions
onto controlled dimensions:
...
## 7. No pre-performance workload gate

CORRECTIVE-C freezes the real workload (payloads, transitions, anchors,
edits, identities) and #31 then begins real measurement. **No
`PERFORMANCE_MATRIX_FREEZE` gate (or any replacement pre-performance
workload gate) stands between CORRECTIVE-C and #31.**

The earlier reservation of such a gate is retired. What remains true:

```text
- SURFACE S/X sweeps are generated only when a material #31 observation
  or hypothesis requires them (R0 rule: observed crossover/cliff →
  explanatory samples), as POST_HOC_EXPLANATORY / POST_HOC_BOUNDARY work;
- controlled work never mutates the frozen real workload membership,
  edits, anchors, or identities;
- the frozen #22 scale points (§5) are unchanged and are owned by #22.
```

No N/B/L/K/F case is generated by CORRECTIVE-C, and none is required
before #31 begins.
...
```text
NO transition taxonomy            (retired line: CORRECTIVE-C freezes it)
NO canonical edits / payloads     (retired line: CORRECTIVE-C freezes them)
NO synthetic N/B/L/K/F cases      (no stage generates them preemptively;
                                   controlled sweeps are observation-driven
                                   #22 follow-up after #31 measurements)
NO H0-H4 timing                   (#31, after the workload freeze)
